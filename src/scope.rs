...
impl GlobalScope {
    ...
    ///
    /// Report undefined variable identifiers as an error.
    ///
    /// ## Example
    ///
    /// ```text
    ...
    pub fn get(&self, pattern: &Pattern) -> Result<ProgNode, Identifier> {
        self.to_pattern().translate(pattern)
    }
    ...
    pub fn get_function(&self, name: &FunctionName) -> Option<&Function> {
        self.functions
            .iter()
        ...
        while let Some(top) = stack.pop() {
            match top {
                ...
                Item::MakePair => {
                    ...
                    output.push(ProgNode::pair_unwrap(expr_l, expr_r));
                }
                Item::MakeTake => {
...
mod tests {
    ...
    fn pattern_translate() {
        ...
        let env = Pattern::array([a.clone(), b.clone(), c.clone()]).unwrap();
